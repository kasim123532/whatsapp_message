...
router.post("/", async (req, res) => {
  ...
  try {
    const existing = await prisma.proxy.findMany({ where: { value: { in: lines } } });
    const existingValues = new Set(existing.map((p) => p.value));
    const newLines = lines.filter((line) => !existingValues.has(line));

    if (newLines.length > 0) {
      await prisma.proxy.createMany({ data: newLines.map((value) => ({ value })) });
    }

    res.status(201).json({ added: newLines.length, skipped: lines.length - newLines.length });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
